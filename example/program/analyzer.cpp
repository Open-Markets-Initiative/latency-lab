...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        description.add_options()
          ...
          (analyzer::file::option, boost::program_options::value<std::string>(), analyzer::file::note);
        ...
        if (values.count("verbose") != 0) {
            std::cout << "Omi Example Analyzer Results" << std::endl;
        }

        // Match files
        const auto wireshark = matcher{ inbounds, outbounds };
        const auto deltas = wireshark.events.deltas().values();
        const auto results = omi::analysis::results{ deltas };
        if (values.count("verbose") != 0) { std::cout << results; }
