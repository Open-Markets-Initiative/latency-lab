...
struct events {
  ...
  //// Construction ///////////////
    ...
    explicit events(const std::string &inbounds, const std::string &outbounds)
      : events{ event::database<inbound>::read(inbounds), event::responses<outbound>::read(outbounds) } {}
    ...
    explicit events(const event::database<inbound> &inbounds, const event::responses<outbound> &outbounds) {
